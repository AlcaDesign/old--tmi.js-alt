...
class Client extends EventEmitter {
	...
	_onOpen() {
		...
		this._sendRaw(`PASS ${ident.password}`);
	...
	_handleMessage(cmd='', params=[], tags={}, prefix={}, raw='', interp={}) {
		...
		this.emit('unknowncommand', cmdName, ...args);
		return false;
	}
	
	hasJoinedChannel(channel = '') {
		channel = utils.username(channel);
		return !_.has(this.channels, channel);
	}
	
	_updateChannel(channel = '', opts = {}) {
		channel = utils.username(channel);
		if(!this.hasJoinedChannel(channel)) {
			this.channels[channel] = {
				userstate: opts.userstate || {},
				roomstate: opts.roomstate || {},
				hosting: opts.hosting || null,
				moderators: new Set(opts.moderators)
			};
		}
		else {
			_.merge(this.channels[channel], opts);
		}
	}
}
